...
steps = 100;
tau = 10^(-4);        % in seconds
D_free_disk = 100;   % diffusivity in nm^2/s 
...
%% Bond lifetimes using Conformation Bond Dynamics
Num_conformation = 1;             % number of conformation states
conformation_diffusivity = 0.034;  % conformation diffusivity 

num_samples = 20000;   % Number of lifetimes to generate
[bond_lifetimes] = distribution_bond_lifetime_expon(Num_conformation, conformation_diffusivity, num_samples);

simulation_timestep = 10^(-5);            % in seconds 

steps_lifetimes = bond_lifetimes/simulation_timestep;
...
D_free_disk = 10^5;           % diffusivity in nm^2/s 
No_timesteps = 300 * 10^5;      
del_t_sampling = 5000;
...
for i = 1:No_timesteps
    ...
    curr_time = i;
    bond_form_prob = 0.01;  % bond formation probability
    ...
    if mod(i,tic_loop) == 0
        disp(['No. time steps elapsed in s:' num2str((i/tic_loop)* 0.01)]);
    end
    
    % save workspace after every 10 seconds interval
    if mod(i, (10^3 * del_t_sampling)) == 0
        outputname = ['data_sim_nb3_af005_N1C0034_' num2str(i) '.mat'];     % change the name of file
        save(outputname)
    end
