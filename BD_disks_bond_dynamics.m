...
function [updated_disks_stat, clusters_ids] = BD_disks_bond_dynamics(No_disks,disks_stat, XX, YY, dia_disk, tau, D_free_disk, bond_form_prob, bondpos, bond_lifetime, bond_formtime, steps_lifetimes, curr_time, ncols,ncol_bonds,ncol_bond_lifetime,ncol_bond_formtime)
dis_factor = 20;
...
for ii = 1:No_disks
    ...
    if mean(disks_stat(ii, bondpos(1,1):bondpos(1,length(bondpos)))) == 0
        [disks_stat] = BD_isolated_disks(ii,disks_stat,D_free_disk,tau,XX,YY,dia_disk);
    end
end
...
for ii = 1:No_disks
    ...
    for jj = 1:No_disks
        if ii ~= jj 
            ...
            id_d2 = disks_stat(jj,1);
            posx_d2 = disks_stat(jj,2);
            posy_d2 = disks_stat(jj,3);
            [disks_stat] = Bond_form_ISB(id_d1,posx_d1,posy_d1,id_d2,posx_d2,posy_d2, disks_stat,bondpos,ncols,ncol_bonds,ncol_bond_lifetime,ncol_bond_formtime,steps_lifetimes,curr_time, dia_disk,dis_factor,bond_form_prob);
        end
    end
end
...
for aa = 1:No_disks
    [disks_stat] = Bond_break_lifetime(aa,bondpos,disks_stat,bond_lifetime,bond_formtime,curr_time);
end
...
for m = 1:length(clusters_ids)    
    ...
    if isempty(ids) == 0 && numel(ids) > 1
        ...
        [disks_stat] = BD_Clusters_pos_update(ids,cluster_disks_pos_old,disks_stat,D_free_disk,dia_disk,tau,XX,YY);        
    end    
end

%% interactions among clusters and disks across periodic boundaries
% perform bond formation across periodic boundaries 
% for disks (in a clusters) whose coordinates lie inside simulation box

% segregate disks belonged to clusters who are outside
diskid_out = [];
disks_stat_out = disks_stat;
[diskid_out,disks_stat_out] = disks_clusters_outside(No_disks,disks_stat_out,XX,YY,diskid_out);

[disks_stat] = Bond_form_cluster_boundaryinteraction(No_disks,diskid_out,disks_stat_out,disks_stat,dia_disk, dis_factor, bondpos,bond_form_prob, steps_lifetimes, curr_time, ncols, ncol_bonds,ncol_bond_lifetime,ncol_bond_formtime);
