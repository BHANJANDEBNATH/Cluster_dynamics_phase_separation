...
function [condition_cluster] = overlap_check_clusters(ids,update_cluster_disks_pos,disks_stat,dia_disk,neighborList, XX, YY)
    ...
    [r c] = size(sc);
        ...
        for j = 1:r
            if imp ~= sc(j,1)
               ...
               dy = dy - round(dy / YY) * YY;  % Periodic boundary condition in y
               % distance
               dis = sqrt(dx^2 + dy^2);
               if dis > dia_disk
